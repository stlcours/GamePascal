...
begin
  ...
  FBackground[2] := Bitmap_Load(Archive, 'arc/backgrounds/spacelayer1.png',
    @BLACK);
  FBackground[3] := Bitmap_Load(Archive, 'arc/backgrounds/spacelayer2.png',
    @BLACK);
...
begin
  ...
  begin
    SmoothMove(FPlayer.TurnSpeed, PLAYER_TURN_ACCEL * aDeltaTime,
      PLAYER_TURN_MAX, PLAYER_TURN_DRAG * aDeltaTime);
  end
  ...
  begin
    SmoothMove(FPlayer.TurnSpeed, 0, PLAYER_TURN_MAX, PLAYER_TURN_DRAG *
      aDeltaTime);
  end;
  ...
  begin
    // update world
    FViewport.Dir.X := (FViewport.Dir.X + (FPlayer.WorldPos.X -
      FViewport.Fixoffset.X - FViewport.Pos.X + FViewport.RunAhead.X *
      FPlayer.Dir.X) * FViewport.Move) / FViewport.Bounce;
    FViewport.Dir.Y := (FViewport.Dir.Y + (FPlayer.WorldPos.Y -
      FViewport.Fixoffset.y - FViewport.Pos.Y + FViewport.RunAhead.Y *
      FPlayer.Dir.Y) * FViewport.Move) / FViewport.Bounce;
    FViewport.Pos.X := FViewport.Pos.X + FViewport.Dir.X;
    FViewport.Pos.Y := FViewport.Pos.Y + FViewport.Dir.Y;

    // update FPlayer
    FPlayer.ScreenPos.X := FPlayer.WorldPos.X - FViewport.Pos.X + cScreenWidth /
      2;
    FPlayer.ScreenPos.Y := FPlayer.WorldPos.Y - FViewport.Pos.Y + cScreenHeight
      / 2;
  end;
...
begin
  ...
  // render FBackground
  Bitmap_DrawTiled(FBackground[0], -(FViewport.Pos.X / 1.9), -(FViewport.Pos.Y /
    1.9));
  Display_SetBlendMode(Blend_AdditiveAlpha);
  Bitmap_DrawTiled(FBackground[1], -(FViewport.Pos.X / 1.9), -(FViewport.Pos.Y /
    1.9));
  Display_RestoreDefaultBlendMode;
  Bitmap_DrawTiled(FBackground[2], -(FViewport.Pos.X / 1.6), -(FViewport.Pos.Y /
    1.6));
  Bitmap_DrawTiled(FBackground[3], -(FViewport.Pos.X / 1.3), -(FViewport.Pos.Y /
    1.3));

  Origin.X := 0.50;
  Origin.Y := 0.50;
  Bitmap_Draw(FPlanet, -(FViewport.Pos.X / 1.0) + (cScreenWidth),
    -(FViewport.Pos.Y / 1.0) + (cScreenHeight), nil, @Origin, nil, 0.0, WHITE,
    False, False);

  // render FPlayer
  Sprite_DrawImage(FSprite, FPlayer.Frame, 0, FPlayer.ScreenPos.X,
    FPlayer.ScreenPos.Y, @Origin, nil, FPlayer.Angle, WHITE, False, False, False);

  // draw avatar
  Bitmap_Draw(FAvatar, cScreenWidth - 64, cScreenHeight - 64, nil, @Vector(0.50,
    0.50), @Vector(0.15, 0.15),
...
begin
  ...
  PrintHudText(ORANGE, Align_Left, 12, 'Pos         [X:%7.3f, Y:%7.3f]',
    [FPlayer.WorldPos.X, FPlayer.WorldPos.Y]);
